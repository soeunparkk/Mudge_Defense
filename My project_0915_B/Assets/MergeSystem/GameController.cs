...
public class GameController : MonoBehaviour
{
    ...
    void SendRayCast()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            {//빈 슬롯에 아이템을 배치
                slot.CreateItem(carryingItem.slotld);   //잡고 있는것 슬롯 위치에 생성
    ...
    {//병합 함수
        ...
        slot.CreateItem(carryingItem.itemld + 1);       //슬롯에 다음 번호 물체 생성
    ...
    {//아이템 배치 실패 시 실행
        var slot = GetSlotById(carryingItem.slotld);        //슬롯 위치 확인
